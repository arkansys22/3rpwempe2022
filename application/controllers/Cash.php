...
class Cash extends CI_Controller
{
  ...
  public function index()
	{
		...
		if ($this->session->level == '1') {
			cek_session_akses('cash', $this->session->id_session);

		} elseif ($this->session->level == '2') {
			cek_session_akses_admin('cash', $this->session->id_session);

		} elseif ($this->session->level == '3') {
			cek_session_akses_direktur('cash', $this->session->id_session);

		} elseif ($this->session->level == '4') {
			cek_session_akses_general_manager('cash', $this->session->id_session);

		}elseif ($this->session->level == '5') {
			cek_session_akses_manager('cash', $this->session->id_session);

		}elseif ($this->session->level == '6') {
			cek_session_akses_supervisor('cash', $this->session->id_session);

		}elseif ($this->session->level == '7') {
			cek_session_akses_staff('cash', $this->session->id_session);

		}else {
			redirect('aspanel/home');
		}
		$this->load->view('backend/data_cash/v_daftar', $data);
	}
  public function storage_bin()
  {
    ...
    if ($this->session->level=='1'){
      ...
      $data['record'] = $this->Crud_m->view_join_where_ordering_field2('finance_cash','finance_cash_level','finance_cash_kategori','finance_cash_level_id',array('finance_cash_status'=>'0'),'finance_cash_id','ASC');
        ...
        $this->load->view('backend/data_cash/v_daftar_hapus', $data);
  }
  public function add()
  {
    ...
    if (isset($_POST['submit'])){
                    $data = array(
                            ...
                            'finance_cash_session'=>md5('Cash'.$this->input->post('finance_cash_bizacc') .hari_ini(date('w')) .date('Y-m-d') .date('H:i:s') ),
            ...
            if ($this->session->level=='1'){
              ...
              $this->load->view('backend/data_cash/v_tambahkan', $data);
            }elseif ($this->session->level=='2'){
              cek_session_akses_admin('cash',$this->session->id_session);
              $data['currency'] = $this->Crud_m->view_ordering('currency','id','asc');
              $this->load->view('backend/data_cash/v_tambahkan', $data);
  ...
  public function update()
  {
    ...
    if (isset($_POST['submit'])){
      ...
      if ($this->session->level=='1'){
        ...
        cek_session_akses_bod('cash',$this->session->id_session);
        $proses = $this->As_m->edit('finance_cash', array('finance_cash_session' => $id))->row_array();
      }else{
        redirect(base_url());
      }
      ...
      $this->load->view('backend/data_cash/v_update', $data);
    }
  }
  ...
  public function acc()
  {
  		...
  		$this->load->view('backend/data_cash/v_daftar_acc', $data);
  	}
  public function update_acc()
  {
      ...
      if (isset($_POST['submit'])){
        ...
        $data['currency'] = $this->Crud_m->view_ordering('currency','id','DESC');
        $this->load->view('backend/data_cash/v_update_acc', $data);
      }
    }
